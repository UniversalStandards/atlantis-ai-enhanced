...
describe("Day-7 release readiness composition", () => {
  ...
  it("rejects operational evidence identity reuse across deployment, rollback, and burn-in phases", () => {
    expect(() => composeDay7ReleaseReadiness({ candidateIdentity: candidate, deployment: deployment(), rollback: { ...rollback(), compatibilityEvidence: ["artifact-1"] }, burnIn: burnIn(), independentGates: gates() })).toThrow("operational evidence identity artifact-1 is reused across deployment and rollback");
    expect(() => composeDay7ReleaseReadiness({ candidateIdentity: candidate, deployment: deployment(), rollback: rollback(), burnIn: { ...burnIn(), ownershipEvents: ["release-1"] }, independentGates: gates() })).toThrow("operational evidence identity release-1 is reused across deployment and burn-in");
    expect(() => composeDay7ReleaseReadiness({ candidateIdentity: candidate, deployment: deployment(), rollback: rollback(), burnIn: { ...burnIn(), persistenceUncertaintyEvents: ["readback-1"] }, independentGates: gates() })).toThrow("operational evidence identity readback-1 is reused across rollback and burn-in");
  });
