...
describe("HarnessRuntime", () => {
  ...
  it("does not dispatch a tool after policy evaluation consumes the remaining deadline budget", async () => {
    const execute = vi.fn(async () => ({ status: "succeeded", output: { ok: true } } as const));
      ...
      tools: [
        {
          name: "echo",
          capability: "read-only",
          description: "should never dispatch after deadline",
          schema: echoSchema,
          execute,
        } satisfies ToolDescriptor,
      ],
    ...
    const result = await runtime.run({
      ...
      plan: async () => ({
        summary: "Attempt echo",
        rationale: "Policy await must be budget-accounted before dispatch",
        desiredOutcome: "No tool dispatch",
        toolName: "echo",
        input: { message: "hello" },
        metadata: {},
      }),
    ...
    expect(policy.evaluate).toHaveBeenCalledOnce();
    expect(execute).not.toHaveBeenCalled();
    expect(result.workDelta.usage.durationMs).toBe(6);
  });
  ...
  it("fails closed when approval resolution cannot be recorded or validated", async () => {
    const execute = vi.fn(async () => ({ status: "succeeded", output: { ok: true } } as const));

    const failingResolver = createRuntime({
      tools: [
        {
          name: "dangerous",
          capability: "write",
    ...
    const failingResolverResult = await failingResolver.runtime.run({
      input: { request: "hello" },
      budget: budget(),
      inspect: async () => ({ startState: { phase: "inspected" } }),
      plan: async () => ({
        summary: "Attempt dangerous tool",
        rationale: "Approval backend errors must fail closed",
        desiredOutcome: "No tool call",
        toolName: "dangerous",
        input: { message: "hello" },
        metadata: {},
    ...
    expect(failingResolverResult.terminalState).toBe("approval_denied");
    expect(execute).not.toHaveBeenCalled();
    expect(failingResolverResult.workDelta.approvalDecisions[0]).toMatchObject({
      outcome: "required",
      failure: {
    ...
    const invalidReceipt = createRuntime({
      tools: [
        {
          name: "dangerous",
          capability: "write",
    ...
    const invalidReceiptResult = await invalidReceipt.runtime.run({
      input: { request: "hello" },
      budget: budget(),
      inspect: async () => ({ startState: { phase: "inspected" } }),
      plan: async () => ({
        summary: "Attempt dangerous tool",
        rationale: "Invalid approval receipts must fail closed",
        desiredOutcome: "No tool call",
        toolName: "dangerous",
        input: { message: "hello" },
        metadata: {},
    ...
    expect(invalidReceiptResult.terminalState).toBe("approval_denied");
    expect(execute).not.toHaveBeenCalled();
    expect(invalidReceiptResult.workDelta.approvalDecisions[0]).toMatchObject({
      outcome: "required",
      failure: {
        code: "approval_resolution_error",
        kind: "deterministic",
  ...
  it("does not dispatch a protected tool after approval evaluation consumes the remaining deadline budget", async () => {
    const execute = vi.fn(async () => ({ status: "succeeded", output: { ok: true } } as const));
    const clock = new DeterministicClock();
    const approvals = {
      resolve: vi.fn(async (request: ApprovalRequest): Promise<ApprovalResolution> => {
    ...
    const result = await runtime.run({
      ...
      plan: async () => ({
        summary: "Attempt dangerous tool",
        rationale: "Approval await must be budget-accounted before dispatch",
        desiredOutcome: "No tool call",
        toolName: "dangerous",
        input: { message: "hello" },
        metadata: {},
  ...
  it("enforces lifecycle-reported token usage before any tool dispatch", async () => {
    const execute = vi.fn(async () => ({ status: "succeeded", output: { ok: true } } as const));
    const { runtime } = createRuntime({
      tools: [
        {
          name: "echo",
          capability: "read-only",
          description: "should not dispatch after plan usage exceeds token budget",
          schema: echoSchema,
          execute,
        } satisfies ToolDescriptor,
      ],
    ...
    const result = await runtime.run({
      ...
      inspect: async () => ({ startState: { phase: "inspected" } }),
      plan: async ({ context }) => {
        context.reportUsage({ inputTokens: 3, outputTokens: 4 });
        return {
          summary: "Attempt echo",
          rationale: "Lifecycle token usage should be budgeted before dispatch",
          desiredOutcome: "No tool call",
          toolName: "echo",
          input: { message: "hello" },
          metadata: {},
    ...
    expect(result.terminalState).toBe("budget_exhausted");
    expect(execute).not.toHaveBeenCalled();
    expect(result.workDelta.usage).toMatchObject({ inputTokens: 3, outputTokens: 4 });
    expect(result.workDelta.selectedPlans).toHaveLength(1);
  });

  it("enforces lifecycle-reported cost usage before refine executes", async () => {
    const refine = vi.fn(async () => ({ status: "continue", nextState: { unreachable: true } } as const));
    const { runtime } = createRuntime({
      tools: [
        {
          name: "echo",
    ...
    const result = await runtime.run({
      ...
      plan: async () => ({
        summary: "Attempt echo",
        rationale: "Evaluation-side cost usage should be budgeted before refine",
        desiredOutcome: "Stop after evaluation",
        toolName: "echo",
        input: { message: "hello" },
        metadata: {},
      }),
    ...
    expect(result.workDelta.usage.costUsd).toBe(2);
    expect(result.workDelta.evaluations).toHaveLength(1);
    expect(result.workDelta.attemptedActions[0]?.outcome).toBe("succeeded");
  });

  it("converts lifecycle callback failures into typed terminal results while preserving authoritative evidence", async () => {
    ...
    for (const phase of phases) {
      ...
      const result = await runtime.run({
        ...
        inspect:
          phase === "inspect"
            ...
            : async () => ({ startState: { phase: "inspected" } }),
        plan:
          phase === "plan"
            ? async () => {
            ...
            : async () => ({
                summary: "Attempt echo",
                rationale: "Lifecycle failure coverage",
                desiredOutcome: "Reach the requested phase",
                toolName: "echo",
                input: { message: "hello" },
                metadata: {},
              }),
        observe:
          phase === "observe"
            ...
            : async ({ toolOutput }) => ({ summary: { observed: toolOutput } }),
        evaluate:
          phase === "evaluate"
            ? async () => {
                throw new Error("evaluate exploded");
              }
            : async () => ({ score: 1, passed: true, reasons: ["ok"], metrics: { quality: 1 } }),
        refine:
          phase === "refine"
            ? async () => {
      ...
      expect(result.workDelta.failure).toMatchObject({
        code: `${phase}_failed`,
        kind: "deterministic",
      });
      ...
      if (phase === "inspect" || phase === "plan") {
        ...
        expect(result.workDelta.attemptedActions[0]?.outcome).toBe("succeeded");
        expect(result.workDelta.policyDecisions[0]).toMatchObject({ allowed: true });
      }
    }
  });
  ...
  it("requires one allowed policy decision for each dispatched attempt", async () => {
    let attempts = 0;
    const { runtime } = createRuntime({
      tools: [
        {
          ...
          execute: async (input) => {
            ...
            return { status: "succeeded", output: { echoed: messageOf(input) } } as const;
          },
        } satisfies ToolDescriptor,
      ],
    });
    ...
    const result = await runtime.run({
      ...
      plan: async () => ({
        summary: "Attempt echo",
        rationale: "Need multiple dispatches",
        desiredOutcome: "Succeed after retries",
        toolName: "echo",
        input: { message: "hello" },
        metadata: {},
      }),
      observe: async ({ toolOutput }) => ({ summary: { observed: toolOutput } }),
      evaluate: async () => ({ score: 1, passed: true, reasons: ["ok"], metrics: { retries: 2 } }),
      refine: async ({ toolOutput }) => ({ status: "complete", finalState: { completed: true }, output: toolOutput }),
    });

    const invalidWorkDelta = JSON.parse(serializeWorkDelta(result.workDelta)) as Record<string, unknown>;
    invalidWorkDelta.policyDecisions = (
      invalidWorkDelta.policyDecisions as Array<Record<string, unknown>>
    ).slice(0, 2);
    expect(() => validateWorkDelta(invalidWorkDelta)).toThrow(InvalidHarnessDataError);
  });

  it("rejects sparse arrays during normalization before stable serialization", () => {
