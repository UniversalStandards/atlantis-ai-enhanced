...
export async function createTrackerProjectedSource(
  input: TrackerProjectionSourceInput,
): Promise<TrackerProjectedSource> {
  ...
  const projectionVersion = input.projectionVersion.trim();
  ...
  return {
    ...
    policyDecision,
    canonicalProjection,
    sourceRevision: `sha256:${await sha256Hex(canonicalProjection)}`,
  };
}
...
export function validateTrackerProjectionVersion(
  ...
  const normalizedActualVersion = actualVersion.trim();
  const normalizedSupportedVersions = supportedVersions.map((version) =>
    version.trim(),
  );

  return normalizedSupportedVersions.includes(normalizedActualVersion)
    ? { compatible: true }
    : {
        compatible: false,
        reason: `Projection version ${normalizedActualVersion} is not supported`,
      };
}
...
>(
  ...
  const source = await createTrackerProjectedSource(request.source);
  ...
  try {
    ...
    if (shouldClaimIdempotency && request.idempotencyStore && !writeStarted) {
      await request.idempotencyStore.abandon(idempotencyKey);
      idempotencyClaimed = false;
    }

    const failureResult: TrackerReconciliationResult<TPlanningContext> = {
      status: "failed",
      trigger: request.trigger,
      idempotencyKey,
      source,
...
function shouldPersistIdempotencyResult(
  ...
  return status === "applied" || status === "noop" || status === "failed";
}
