...
if (!/^[^/]+\/[^/]+$/.test(repository)) throw new Error(`Invalid ATLANTIS_REPOSITORY: ${repository}`);
if (!Number.isInteger(pullRequest) || pullRequest <= 0) throw new Error(`Invalid ATLANTIS_RELEASE_PR: ${pullRequest}`);
if (!Number.isInteger(requiredAppId) || requiredAppId <= 0) throw new Error(`Invalid ATLANTIS_REQUIRED_APP_ID: ${requiredAppId}`);

const headers = { Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "atlantis-release-control-evidence" };
...
function evaluateRuleset(ruleset, branchName) {
  ...
  const requiredCheckConfigured = requiredChecks.some((check) => check?.context === requiredCheck && Number(check?.integration_id) === requiredAppId);
  ...
  const bypassFree = bypassActors.length === 0;
  return {
    id: ruleset?.id ?? null, name: ruleset?.name ?? null, enforcement: ruleset?.enforcement ?? null, target: ruleset?.target ?? null,
    ...
    bypass_free: bypassFree,
    release_control_enforced: applies && approvingReviewsRequired && requiredCheckConfigured && bypassFree,
  };
}
...
function runSelfTest() {
  ...
  const wrongApp = evaluateRuleset({ ...fixture, rules: fixture.rules.map((rule) => rule.type === "required_status_checks" ? { ...rule, parameters: { required_status_checks: [{ context: requiredCheck, integration_id: requiredAppId + 1 }] } } : rule) }, "main");
...
const matchingChecks = (checkRunsResult.body?.check_runs ?? []).filter((run) => run?.name === requiredCheck && Number(run?.app?.id) === requiredAppId);
...
const evidence = {
  ...
  observed_matching_checks: matchingChecks.map((run) => ({ id: run.id, name: run.name, app_id: run.app?.id, status: run.status, conclusion: run.conclusion, details_url: run.details_url })),
  ...
  classic_branch_protection: classicEvidence, evaluated_rulesets: evaluatedRulesets, enforcing_ruleset_ids: enforcingRulesets.map((ruleset) => ruleset.id),
};
