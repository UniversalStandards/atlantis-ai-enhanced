...
| --- | --- | --- | --- |
| Regression suite | Frozen-lockfile install, SEC-20 supply-chain gates, contracts/event-store typecheck and tests | Latest verified branch-head baseline is **370/370 contracts + 530/530 event-store = 900/900 tests**. Exact-head evidence anchor `b43937efbd99a56921f76423198b6a1fa1518622` passed Contracts run `33307899674`; frozen install, SEC-20 gates, both TypeScript workspaces, read-only Actions permissions, and zero vulnerability findings at every severity remain green. This CI evidence does not manufacture additional operational capability. | Must remain 100% green on the release candidate |
