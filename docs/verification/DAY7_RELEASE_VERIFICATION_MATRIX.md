...
| --- | --- | --- | --- |
| Regression suite | Frozen-lockfile install, SEC-20 supply-chain gates, contracts/event-store typecheck and tests | Latest verified runtime baseline is **370/370 contracts + 529/529 event-store = 899/899 tests**. Exact-head evidence anchor `bfcf82c63b02d0f586577a163ff43ab23135802f` passed Contracts run `33291575590`; frozen install, SEC-20 gates, both TypeScript workspaces, read-only Actions permissions, and zero vulnerability findings at every severity remain green. This CI evidence does not manufacture additional operational capability. | Must remain 100% green on the release candidate |
