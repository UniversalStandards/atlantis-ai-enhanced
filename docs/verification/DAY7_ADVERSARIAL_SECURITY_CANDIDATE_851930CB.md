...
| SEC-14 Secret leakage | PASS at inspected CI/component level | Workflow logs expose read-only permission names but redact token values; diagnostic ownership tests passed. This is not a production-secret scan. |
...
| SEC-20 Dependency / CI supply-chain compromise | AMBER / partial | Frozen lockfile passed; pnpm reported ignored `esbuild` build scripts; GitHub Actions were resolved to concrete action SHAs during the run. No approved vulnerability scanner/SBOM/signature-verification campaign was executed, so zero-critical supply-chain status is not proven. |
...
- BLOCKED scenarios: **3** (`SEC-17`, `SEC-18`, `SEC-19`).
- Partial/AMBER scenarios: **1** (`SEC-20`).
